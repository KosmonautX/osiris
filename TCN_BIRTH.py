...
def endtoend(target, cols, raw, sector, depth=4, batch_size = 8, patience =120,learner=0.002):
    #--------------------------------------------------------------BOLLINGER BAND---------------------------------------
    ...
    opt = torch.optim.Adam(model.parameters(), lr=learner)
    ...
    plt.plot(raw['total'].head(len(raw)-len(preds)+1), label='Training Data')
